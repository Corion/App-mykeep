...
try {
  self.workbox.v['workbox:core:3.0.0-alpha.1'] = 1;
...
var messages = {
  ...
  'incorrect-type': ({ expectedType, paramName, moduleName, className,
    ...
    return `The parameter '${paramName}' passed into ` + `'${moduleName}.${className}.${funcName}()' must be of type ` + `${expectedType}.`;
  },

  'incorrect-class': ({ expectedClass, paramName, moduleName, className,
    funcName }) => {
    if (!expectedClass || !paramName || !moduleName || !className || !funcName) {
      throw new Error(`Unexpected input to 'incorrect-class' error.`);
    }
    return `The parameter '${paramName}' passed into ` + `'${moduleName}.${className}.${funcName}()' must be an instance of ` + `class ${expectedClass.name}.`;
  },
